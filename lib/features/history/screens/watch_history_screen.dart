...
class _WatchHistoryScreenState extends ConsumerState<WatchHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final movies = state.movies.where((m) {
      final matchQuery =
          _search.isEmpty ||
          m.title.toLowerCase().contains(_search.toLowerCase());
      final matchGenre =
          _genreFilter == null || m.genres.contains(_genreFilter);
      return matchQuery && matchGenre;
    }).toList();
    final allGenres = <String>{for (final m in state.movies) ...m.genres}
      ..removeWhere((g) => g.trim().isEmpty);
    return Scaffold(
      appBar: AppBar(
            ...
            child: Row(
              children: [
                ...
                DropdownButton<String?>(
                  ...
                  onChanged: (v) =>
                      setState(() => _genreFilter = v == '__ALL__' ? null : v),
                  items: [
        ...
        child: Builder(
          builder: (context) {
            ...
            if (state.error != null && state.movies.isEmpty) {
              return ErrorEmptyState.error(
                message: state.error!,
                onRetry: () =>
                    ref.read(watchHistoryProvider.notifier).loadInitial(),
              );
            }
            if (movies.isEmpty) {
              return ListView(
                children: [
                  ...
                  ErrorEmptyState.empty(
                    title: 'No watch history yet',
                    message:
                        'Start swiping and your liked movies will appear here.',
                    icon: Icons.history,
                  ),
            ...
            if (_grid) {
              return GridView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  final m = movies[i];
                  return _HistoryGridTile(movie: m);
                },
              );
            } else {
              return ListView.separated(
                ...
                itemBuilder: (_, i) {
                  ...
                  final m = movies[i];
                  return _HistoryListTile(movie: m);
                },
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          ...
          final movie = await showDialog<Movie?>(
            context: context,
            builder: (ctx) {
              return AlertDialog(
                  ...
                  child: SizedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text(
                          'Enter the ID of a known mock movie (e.g., 1001, 1002, 1003).',
                        ),
                      ],
                    ),
                  ),
          ...
          if (movie != null) {
            await ref.read(watchHistoryProvider.notifier).addMovie(movie);
          }
        },
      ),
    );
...
class _HistoryGridTile extends StatelessWidget {
  const _HistoryGridTile({required this.movie});
  final Movie movie;
  @override
  Widget build(BuildContext context) {
    return InkWell(
