...
class MovieStackNotifier extends StateNotifier<MovieStackState> {
  MovieStackNotifier(this._ref) : super(const MovieStackState()) {
    _init();
  }
  final Ref _ref;
  ...
  List<Movie> _applyPreferenceWeighting(List<Movie> movies) {
    final authState = _ref.read(authProvider);
    final prefsGenres = authState.user?.preferredGenres ?? [];
    if (prefsGenres.isEmpty) return movies;
    final genreSet = prefsGenres.map((g) => g.toLowerCase()).toSet();
    final scored = movies.map((m) {
      final match = m.genres
          .where((g) => genreSet.contains(g.toLowerCase()))
          .length;
      return MapEntry(m, match);
    }).toList();
  ...
  void _ensureStackFilled() {
    ...
    if (needed > 0 && state.buffer.isNotEmpty) {
      ...
      state = state.copyWith(
        stack: [...toAdd.reversed, ...state.stack],
        buffer: state.buffer.skip(toAdd.length).toList(),
      );
    }
  ...
  List<Movie> _applyActiveFilters(List<Movie> movies) {
    final genres = _ref.read(selectedGenresProvider);
    final minRating = _ref.read(minRatingFilterProvider);
    if (genres.isEmpty && minRating == 0) return movies;
    return movies.where((m) {
      final matchGenres =
          genres.isEmpty || m.genres.any((g) => genres.contains(g));
      final matchRating = m.voteAverage >= minRating;
    ...
    try {
      switch (interaction) {
        case UserInteraction.liked:
          await repo.likeMovie(movie.id);
          break;
        case UserInteraction.disliked:
          await repo.dislikeMovie(movie.id);
          break;
        case UserInteraction.superLiked:
          await repo.superLikeMovie(movie.id);
          break;
  ...
  Future<void> _persistInteraction(
    ...
    if (interaction == UserInteraction.none) return;
    final idStr = movieId.toString();
    if (interaction == UserInteraction.liked ||
        interaction == UserInteraction.superLiked) {
      final liked = await _storage!.getLikedMovies();
      if (!liked.contains(idStr)) {
        liked.add(idStr);
        await _storage!.saveLikedMovies(liked);
      }
    } else if (interaction == UserInteraction.disliked) {
      final disliked = await _storage!.getDislikedMovies();
      if (!disliked.contains(idStr)) {
        disliked.add(idStr);
        await _storage!.saveDislikedMovies(disliked);
      }
    }
  }

  Future<void> _removePersistedInteraction(
    int movieId,
    UserInteraction interaction,
  ) async {
    if (_storage == null) return;
    ...
    if (interaction == UserInteraction.liked ||
      ...
      final disliked = await _storage!.getDislikedMovies();
      disliked.remove(idStr);
      await _storage!.saveDislikedMovies(disliked);
    }
  }
}

final movieStackProvider =
    StateNotifierProvider<MovieStackNotifier, MovieStackState>(
      (ref) => MovieStackNotifier(ref),
    );

class _SwipeRecord {
  final Movie movie;
  final UserInteraction interaction;
  _SwipeRecord({required this.movie, required this.interaction});
}
