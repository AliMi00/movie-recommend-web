...
class _SettingsScreenState extends ConsumerState<SettingsScreen>{
  ...
  Future<void> _resetData() async {
    ...
    if(mounted){ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('User data cleared'))); }
  }
  ...
  Future<void> _resendVerification() async {
    ...
    final sent = await ref.read(authProvider.notifier).resendVerificationEmail(email);
    if (!mounted) return;
  ...
  Future<void> _quickEditGenres() async {
    ...
    final allGenres = ['Action','Adventure','Drama','Romance','Comedy','Science Fiction','Horror','Thriller'];
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text('Preferred Genres', style: Theme.of(ctx).textTheme.titleMedium),
                      ],
                    ),
                      ...
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            onPressed: () {
                              ref.read(selectedGenresProvider.notifier).state = selected;
  ...
  @override Widget build(BuildContext context){
    ...
    if (!emailVerified)
      ListTile(
        leading: const Icon(Icons.mark_email_unread_outlined, color: AppColors.error),
        title: const Text('Verify your email'),
        subtitle: const Text('Tap to resend the verification link'),
        onTap: _resendVerification,
      ),
    ListTile(leading: const Icon(Icons.flash_on), title: const Text('Quick Edit Preferred Genres'), subtitle: const Text('Inline adjust'), onTap: _quickEditGenres),
    ListTile(leading: const Icon(Icons.replay_rounded), title: const Text('Replay Intro'), subtitle: const Text('Show the welcome walkthrough again'), onTap: _replayIntro),
    ListTile(leading: const Icon(Icons.cleaning_services_outlined), title: const Text('Clear User Data'), subtitle: const Text('Reset liked/disliked/history and watchlist'), onTap: _resetData, trailing: const Icon(Icons.delete_forever, color: Colors.redAccent)),
    const Divider(),
    const Padding(
    ...
    const Divider(),
    ListTile(title: const Text('About'), subtitle: const Text('Version 1.0.0 • Credits'), leading: const Icon(Icons.info_outline), trailing: const Icon(Icons.chevron_right_rounded), onTap: _showAboutDialog),
    const SizedBox(height:24),
  ])); }
}
