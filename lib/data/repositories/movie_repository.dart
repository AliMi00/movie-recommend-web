...
class MockMovieRepository implements MovieRepository {
  ...
  Future<LocalStorageService> get _ensureStorage async =>
  ...
  Future<List<Movie>> _loadMovies() async {
    ...
    try {
      final String jsonString = await rootBundle.loadString(
        AppConstants.mockDataAssetPath,
      );
  ...
  List<Movie> _getHardcodedMovies() {
    return [
      const Movie(
        ...
        overview:
            "An epic space adventure following a crew of explorers as they venture beyond the known galaxy to discover new worlds and face unimaginable dangers in the depths of space.",
      ...
      const Movie(
        ...
        overview:
            "A romantic comedy about a writer who mysteriously finds himself transported back to the 1920s literary scene in Paris every night at midnight.",
  ...
  Future<List<Movie>> getRecommendations({
    ...
    final availableMovies = movies
        .where((movie) => !swipedMovieIds.contains(movie.id))
        .toList();
  ...
  Future<List<Movie>> searchMovies(String query) async {
    ...
    return movies.where((movie) {
      return movie.title.toLowerCase().contains(lowercaseQuery) ||
          ...
          movie.cast.any(
            (actor) => actor.toLowerCase().contains(lowercaseQuery),
          ) ||
          movie.genres.any(
  ...
  Future<List<Movie>> getLikedMovies() async {
    ...
    return List.from(_likedMovies)..sort(
      (a, b) =>
  ...
  Future<List<Movie>> getWatchHistory() async {
    ...
    return List.from(_watchHistory)..sort(
      (a, b) => (b.watchedAt ?? DateTime.now()).compareTo(
        a.watchedAt ?? DateTime.now(),
      ),
    );
  }
  ...
  Future<void> addToWatchHistory(int movieId) async {
    ...
    try {
      ...
      await storage.saveWatchHistory(
        _watchHistory.map((m) => m.id.toString()).toList(),
      );
    } catch (_) {
...
class ApiMovieRepository implements MovieRepository {
  ...
  Future<void> likeMovie(int movieId) async {
    await _dio.post(
      '/interactions/swipe',
      data: {'movieId': movieId, 'action': 'like'},
    );
    final storage = await LocalStorageService.getInstance();
  ...
  Future<void> dislikeMovie(int movieId) async {
    await _dio.post(
      '/interactions/swipe',
      data: {'movieId': movieId, 'action': 'dislike'},
    );
  }
  ...
  Future<void> superLikeMovie(int movieId) async {
    await _dio.post(
      '/interactions/swipe',
      data: {'movieId': movieId, 'action': 'super_like'},
    );
    final storage = await LocalStorageService.getInstance();
    final liked = await storage.getLikedMovies();
    if (!liked.contains(movieId.toString())) {
