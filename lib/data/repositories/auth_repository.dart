...
class ApiAuthRepository implements AuthRepository {
  ...
  User _mapUser(Map<String, dynamic> u) {
    return User(
      ...
      preferredGenres: List<String>.from(u['preferredGenres'] ?? u['preferred_genres'] ?? const []),
      isFirstTimeUser: (u['isFirstTimeUser'] as bool?) ?? (u['is_first_time_user'] as bool?) ?? false,
      minRating: (u['minRating'] as num?)?.toDouble() ?? (u['min_rating'] as num?)?.toDouble() ?? 7.0,
      isDarkTheme: (u['isDarkTheme'] as bool?) ?? (u['is_dark_theme'] as bool?) ?? true,
      ...
      emailVerified: (u['emailVerified'] as bool?) ?? (u['email_verified'] as bool?) ?? true,
      createdAt: u['createdAt'] != null ? DateTime.tryParse(u['createdAt']) : null,
      lastActiveAt: u['lastActiveAt'] != null ? DateTime.tryParse(u['lastActiveAt']) : null,
    );
  }

  Future<void> _persistAuth(User user, {String? access, String? refresh}) async {
    final storage = await LocalStorageService.getInstance();
    await storage.saveAuthUser(user);
    if (!_config.useCookieAuth) {
      if (access != null && access.isNotEmpty) await _secureStorage.write(key: 'access_token', value: access);
      if (refresh != null && refresh.isNotEmpty) await _secureStorage.write(key: 'refresh_token', value: refresh);
    }
  }
  ...
  Future<AuthResult> register(String email, String password, String username) async {
    try {
      ...
      if (userMap != null) {
        ...
        user = User(id: 'me', email: email, username: username, isFirstTimeUser: true);
      }
      
      final access = data['accessToken'] as String? ?? data['access_token'] as String?;
      final refresh = data['refreshToken'] as String? ?? data['refresh_token'] as String?;
      ...
      if (access != null && access.isNotEmpty) {
        ...
        if (loginRes.success && loginRes.user != null) {
          return AuthResult.success(loginRes.user!.copyWith(isFirstTimeUser: true));
        }
        await _persistAuth(user);
        return AuthResult.success(user);
      }
    } on DioException catch (e) {
      final message = _extractErrorMessage(e.response?.data, 'Registration failed');
      return AuthResult.failure(message);
    } catch (e) {
      return AuthResult.failure('An unexpected error occurred');
    }
  ...
  String _extractErrorMessage(dynamic data, String fallback) {
    if (data is Map<String, dynamic>) {
      ...
      } else if (data['error'] is Map<String, dynamic> && data['error']['message'] is String) {
  ...
  Future<void> saveUserPreferences(UserPreferences preferences) async {
    try {
      await _dio.put('/users/me/preferences', data: {
